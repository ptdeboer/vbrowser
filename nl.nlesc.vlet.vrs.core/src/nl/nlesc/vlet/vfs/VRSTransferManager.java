...
import nl.nlesc.vlet.vdriver.vrs.http.HTTPNode;
...
public final class VRSTransferManager
{
            ...
            ICopyInteractor copyInteractor) throws Exception
    {
        ...
        if (node instanceof VLogicalResource)
        {
            ...
            if (storageVrl==null)
                throw new nl.nlesc.vlet.exception.ResourceTypeNotSupportedException("Can not copy logical nodes without a storage location:"+node);
...
//        
	    ...
	    throws Exception
	{
		...
		if ( (VRLUtil.hasSameServer(sourceFile.getLocation(),targetFile.getVRL())) && (isMove))
		{
			...
			if (result==false)
				throw new nl.nlesc.vlet.exception.ResourceCreationFailedException("Rename failed:"+sourceFile);
...
//		}
	 ...
	 */ 
	public boolean doActiveFileTransfer(ITaskMonitor monitor,VNode sourceFile,VNode targetFile, StringHolder reasonH) throws Exception
	{
	   ...
	   // 3rd party from file to remote:
		...
		{    
			...
			if (canTransferToOther)
			{
				...
				logger.infoPrintf("Source can perform active (3rd party) transfer to:%s\n",targetVRL); 
				toStr=reasonH.value; 
	        ...
	        boolean isMove)  throws VlException
	{
	    if (sourceNode.isComposite()) 
	    {
	        ...
	        throw new nl.nlesc.vlet.exception.ResourceTypeMismatchException("Can not copy resource:"+sourceNode);
	    }
	    else
	    {
	        //single source must be stream readable 
            if ((sourceNode instanceof VStreamReadable)==false)
                throw new nl.nlesc.vlet.exception.ResourceTypeMismatchException("Can not read contents from:"+sourceNode);
	    }
...
//	}
			...
			ICopyInteractor interactor) throws Exception
	{
		...
		if ((isMove) && (VRLUtil.hasSameServer(sourceDir.getLocation(),_targetDirVrl) ))
		{
			...
			{
				throw new nl.nlesc.vlet.exception.ResourceCreationFailedException("Rename failed:"+this);
			}
		}
