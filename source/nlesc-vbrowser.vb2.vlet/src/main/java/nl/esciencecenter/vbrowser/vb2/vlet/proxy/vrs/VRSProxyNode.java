...
public class VRSProxyNode extends ProxyNode
{
    ...
    @Override
    protected void doUpdateAttributes(Attribute[] attributes) throws ProxyException {
        if (vnode instanceof VEditable) {
            try {
                ((VEditable)vnode).setAttributes(attributes);
            } catch (VrsException e) {
    ...
    @Override
    protected Map<String, AttributeDescription> doGetAttributeDescriptions(String[] strings) throws ProxyException {
        return new HashMap<>();
    }
    ...
    protected boolean doGetIsEditable() {
        return (vnode instanceof VEditable);
    }
...
//        }
    ...
    {
        // put ResourceConfigIngo back to ServerInfo
        return info;
    }
