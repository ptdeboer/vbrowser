...
import nl.esciencecenter.vlet.actions.ActionMenuMatcher;
...
public class ActionMenu extends JPopupMenu
{
    ...
    private static Vector<JMenuItem> createDynamicActionMenuItemsFor(BrowserController bc, VComponent vcomp, boolean canvasMenu)
	{
    	...
    	if (subMenus!=null)
    	{
    		...
    		{
    			UILogger.infoPrintf(ActionMenu.class," Creating extra submenu:%s\n",subName); 
    			...
    			{
    				UILogger.warnPrintf(ActionMenu.class,"Warning: No dynamic submenu items for MATCHED submenu:%s\n",subName); 
    			}
    		}
    	}
    	else
    		UILogger.debugPrintf(ActionMenu.class,"--- No Matched Submenus ---"); 
	...
	private static Vector<JMenuItem> createDynamicMenuItemsFor(BrowserController bc, VComponent vcomp, boolean canvasMenu,ActionMenuMatcher matcher,String subMenu)
	{
		...
		if ((actionItems==null) || (actionItems.size()<=0))
		{
			...
			UILogger.infoPrintf(ActionMenu.class,"Warning: Matcher didn't return any menu items for matched submenu:%s\n",subMenu); 
			return null;
		}
		...
		for (ActionMenuMapping actionItem:actionItems)
    	{
			...
			UILogger.debugPrintf(ActionMenu.class," - Adding :"+"subMenu"+"/"+name+"(actionName())\n");
    		...
    		{
    			UILogger.errorPrintf(ActionMenu.class,"Either VRS or IViewer must be defined for Dynamic Acion:%s\n",actionName);
    			return null; 
    		}
		...
		UILogger.debugPrintf(ActionMenu.class,"Returning #subMenuItems:"+menuItems.size()+" for subMenu:"+subMenu+"\n"); 
