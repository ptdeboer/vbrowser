...
import nl.esciencecenter.vlet.grid.proxy.GridProxyListener;
import nl.esciencecenter.vlet.gui.GuiPropertyName;
...
import nl.esciencecenter.vlet.gui.actions.ActionMenu;
...
import nl.esciencecenter.vlet.gui.icons.IconViewType;
...
import nl.esciencecenter.vlet.gui.viewers.DefaultNodeViewer;
   ...
   MasterBrowser, ProxyResourceEventListener, ITaskSource 
{
	...
	private void handleAction(VComponent vcomp,final ActionCommand actionCommand,boolean globalAction)
	{
		...
		try
		{
			...
			if (actionDone==false)
			{
				...
				{
					logger.infoPrintf("No resource selected for action:%s\n",actionCommand.actionType);
				}
	...
	private void performEditACL(ProxyNode node)
	{
		try
		{
			nl.esciencecenter.vlet.gui.panels.acldialog.ACLPanel.showEditor(this,node);
		}
		catch (VrsException e)
		{
			handle(e); 
...
//		this.itemviewer = viewer;
	...
	public void setHasActiveTasks(boolean val)
	{
	...
	private void initViewFilter()
	{
		...
		viewFilter.setSortField1(VAttributeConstants.ATTR_RESOURCE_TYPE); 
...
//		TaskMonitorDialog.showTaskMonitorDialog(this.getFrame(), dynamicAction,1000); 
	...
	public void performStartStandaloneViewer(final ActionCommand actionCommand, final ActionContext actionContext) throws VrsException
    {
        ...
        UILogger.infoPrintf(this, "PerformActionCommand = %s\n",methodName);
        UILogger.infoPrintf(this, "ViewerClass          = %s\n",iviewerClassName);
        UILogger.infoPrintf(this, "Selection context    = %s\n",actionContext);
    ...
    {
        UIGlobal.getTaskWatcher().registerTask(actionTask); 
    }

    @Override
    public void notifyTaskStarted(ActionTask actionTask)
    {
        UIGlobal.getTaskWatcher().notifyTaskStarted(actionTask); 
    }
