...
import nl.esciencecenter.vlet.gui.actions.KeyMappings;
...
import nl.esciencecenter.vlet.gui.vbrowser.BrowserJPanel;
        ...
        VContainer
{
    ...
    protected synchronized void uiAddIconItems(final ViewNode[] items) 
    {
        ...
        for (ViewNode pnode:items) // Enumeration<String> keys = proxyNodes.keys();keys.hasMoreElements();) // keys.nextElement()) // int i = 0; i < numNodes; i++)
        {
            ...
            if (pnode==null)
            {
                ...
                UILogger.infoPrintf(this,"*** Warning: child node not in cache, fetching childs of:%s\n",rootNode);
            }
            else
            {
            	// extra filter: 
	...
	private void handle(Throwable e1) 
    {
    	this.masterBrowser.notifyTaskException(null,e1);  
	}
    ...
    private void bgRefreshLabelIcon(final LabelIcon bicon,final VRL oldLoc, final VRL newLoc)
    {
        ...
        {
            // todo caching
        	UILogger.errorPrintf(this,"Warning, new node NOT found:%s\n",newLoc);
        	return; 
        }
	...
	public void setSelected(VRL vrl, boolean combine, boolean value)
	{
		 ...
		 if (bicon==null)
		 {
		     UILogger.debugPrintf(this,"setSelected(): Couln't find icon for:%s\n",vrl); 
		     return; 
         }
    ...
    public void selectRange(VRL vrl, boolean isFirst)
    {
        ...
        if (last==null)
        {
            UILogger.debugPrintf(this,"selectRange(): Couln't find 2d selection marker:%s\n",vrl); 
            return; 
        }
