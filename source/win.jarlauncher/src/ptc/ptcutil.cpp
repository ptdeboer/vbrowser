/*
 * (C) Piter.nl 2010-2013 
 * See  : http://www.piter.nl/source/ptcutil)
 * File : ptcutil.cpp
 * Info :
 *
 ...
 *       - 'mem_' memory methods
 *       - 'str_' plain string methods using char* as type
 *       - 'file_' plain file methods
 * 
...
//============================================================================
// Memory methods: See ptcdefs.hpp for (m)alloc/free macro definitions
//============================================================================
...
/**
 * String duplicate.
 * Also convenient for converting (const char *) to (char *)!
 */
char *Sduplicate(const char *str)
{
    ...
    ASSERT_NOTNULL("Sduplicate():Possible out of memory error. Got NULL from strdup.",
                   result);
    return result;
}
 ...
 */
int Fwrite(const char *file, const void *ptr, size_t elementSize,
        size_t numElements)
{
    if (file == NULL)
        return -1;
