...
//    public static final String VRSFACTORY_IS_VDRIVER   = "vrsfactory.isvdriver";
			...
			boolean recurse, boolean addDirs)
	{
		...
		if (pluginLoc.isFile())
		{
			...
			{    
				logger.logException(ClassLogger.INFO,this,e2,"Error adding url:%s\n",pluginLoc);
			}
		}
...
//		}
			...
			boolean addDirs)
	{
		...
		{
			logger.logException(ClassLogger.ERROR,e1,"Exception for:%s\n",baseLocation);
		}
	...
	public Vector<PluginInfo> loadLocalPlugins(String dirPath, Class<?> pluginClass)
	{
		...
		for (String name : names)
		{
			...
			{
			    logger.logException(ClassLogger.ERROR,this,e,"Exception when loading/initializing loading plugin:%s\n",name);
			} // END try
		} // END for
