...
import nl.esciencecenter.vlet.vrs.vfs.VUnixGroupMode;
import nl.esciencecenter.vlet.vrs.vfs.VUnixUserMode;

/**
 * URI 'ls' tool. Is called from urils script.
 * 
 * @since VLET 1.2
 */
public class URILs
{
    ...
    public void assertValidProxy() throws Exception
    {
        ...
        // throw new
        // Exception("No valid proxy found at location:"+proxy.getProxyFilename());
    ...
    public void ls() throws Exception
    {
        ...
        if (source.isComposite())
        {
            ...
            else
            {
                ...
                verbosePrintln(" - resource is composite type:" + source.getResourceType());
            }
        }
        else
        {
            verbosePrintln(" - resource is non-composite type:" + source.getResourceType());
        ...
        for (VNode node : nodes)
        {
            ...
            else if ((optLongList == false) && (optPrintGuid == true))
            {
                ...
                formatter.format("%36s %s", guidstr, nodeName);
            }
            else if ((optLongList == true) && (optPrintGuid == false))
            {
                formatter.format("%s %9s %9s %9s %16s %s", permstr, uid, gid, sizestr, modstr, nodeName);
            }
            else if ((optLongList == true) && (optPrintGuid == true))
            {
                // standard UUID is 36 characters wide:
                formatter.format("%s %9s %9s %9s %16s %36s %s", permstr, uid, gid, sizestr, modstr, guidstr, nodeName);
            }
    ...
    public String createTimeStr(long time)
    {
        ...
        return "" + year + "-" + Presentation.to2decimals(month) + "-" + Presentation.to2decimals(day) + " "
                + Presentation.to2decimals(hours) + ":" + Presentation.to2decimals(minutes);
