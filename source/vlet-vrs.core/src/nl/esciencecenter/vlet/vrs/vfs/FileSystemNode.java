...
import nl.esciencecenter.vlet.vrs.ResourceSystemNode;
...
public abstract class FileSystemNode extends ResourceSystemNode implements VFileSystem
{
	...
	public VDir getDir(VRL dirVrl) throws VrsException
	{
		...
		if (node==null)
			throw new nl.esciencecenter.vlet.exception.ResourceNotFoundException(" Couldn't get: "+dirVrl); 
		
		if ((node instanceof VDir)==false) 
			throw new nl.esciencecenter.vlet.exception.ResourceTypeMismatchException(" Resource is not a directory: "+ node); 
	...
	public VFile getFile(VRL fileVrl) throws VrsException
	{
		...
		if (node==null)
			throw new nl.esciencecenter.vlet.exception.ResourceNotFoundException(" Couldn't get: "+fileVrl); 
		
		if ((node instanceof VFile)==false) 
			throw new nl.esciencecenter.vlet.exception.ResourceTypeMismatchException(" Resource is not a file:"+fileVrl); 
	...
	public VDir createDir(VRL dirVrl, boolean ignoreExisting) throws VrsException
	{
		...
		if (dir.exists()==true)
		{
			...
			{
				throw new nl.esciencecenter.vlet.exception.ResourceAlreadyExistsException("Directory already exists:"+dir);
			}
			else
			{
				return dir; 
	...
	public VFile createFile(VRL fileVrl, boolean ignoreExisting) throws VrsException
	{
		...
		if (file.exists()==true)
		{
			if (ignoreExisting==false) 
			{
				throw new nl.esciencecenter.vlet.exception.ResourceAlreadyExistsException("File (or path) already exists:"+file);
			}
			else
			{
				return file;  
