...
public final class VFSClient extends VRSClient 
{
    ...
    public  VFile getFile(VRL location) throws VlException
    {
        ...
        if (node==null)
            throw new nl.nlesc.vlet.exception.ResourceNotFoundException("Couldn't get file:"+location);
        
        //
        if (node.exists()==false)
            throw new nl.nlesc.vlet.exception.ResourceNotFoundException("File doesn't exist:"+location);
        
        if (node instanceof VDir)
            throw new nl.nlesc.vlet.exception.ResourceTypeMismatchException("Resource is not file, but a directory:"+location);
