...
import nl.esciencecenter.vlet.gui.dnd.VDragGestureListener;
import nl.esciencecenter.vlet.gui.dnd.VTransferHandler;
import nl.esciencecenter.vlet.gui.proxyvrs.ProxyNode;
import nl.esciencecenter.vlet.gui.table.VTableController.TablePos;
 ...
 * 
 * Uses VRSTableModel as table custom model. Note that the tablePanel can have a different column order as used in the
 * DataModel.
 * 
...
public class TablePanel extends JTable implements VContainer, IPresentable
{
    ...
    static
    {
        logger = ClassLogger.getLogger(TablePanel.class);
    ...
    VRSTableModel getVRSTableModel()
    {
        ...
        else
        {
            ...
            logger.warnPrintf("Table not yet initialized ???\n");
            return null;
        }
    }
    ...
    /**
     * When the autoCreateColumns is on, default cell renderers are creates. This method overrides them
     */
    ...
    // {
    // // for testing:
    // UILogger.logException(this,ClassLogger.ERROR,e,"TablePanel Exception!\n");
    // }
    // }
    ...
    /**
     * UI Save Create table from object matrix. Will autodetect VAttribute matrix, but VAttributes in the same column
    ...
    {
        logger.debugPrintf("uiCreateTable: (I):thread#%d\n", Thread.currentThread().getId());
        // ===
        // ui thread:
        // ===
        ...
        logger.debugPrintf("uiCreateTable: (IIb) is gui thread\n");
        // ==
    ...
    public void insertColumn(String headerName, String newName)
    {
        ...
        if (presentation != null)
            presentation.setPreferredContentAttributeNames(new StringList(newNames));
    ...
    {
        if (w==10)
            throw new Error("Column Width=10!");
    ...
    {
        UILogger.warnPrintf(this, "Fixme:selectAll()");
    }
