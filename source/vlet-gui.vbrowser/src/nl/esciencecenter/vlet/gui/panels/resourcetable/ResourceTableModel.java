...
public class ResourceTableModel extends AbstractTableModel implements Iterable<ResourceTableModel.RowData>
{
	...
	public class RowData
	{
        ...
        private void init(String rowKey, AttributeSet attrs) {
			this.rowKey=rowKey;

			if (attrs!=null) {
                this.rowAttributes = attrs.duplicate(); // empty set!
                this._rowAttributeNames= rowAttributes.createKeyArray();
			} else {
			    this.rowAttributes=new AttributeSet();
			    this._rowAttributeNames= new String[0];
            }
		}

        public String getAttributeName(int colNr) {
            if ((colNr<0) || (colNr>=this._rowAttributeNames.length)) {
                logger.error("Column index out of bound:{} <> {}",colNr,_rowAttributeNames.length);
                return null;
            }
	...
	public boolean setValue(String key,String attrName,String value)
	{
	    synchronized(rows)
        {
            ...
            if (rowIndex<0) {
                logger.error("Row not found:{}",key);
                return false;
            }

            RowData row = this.rows.get(rowIndex);
            if (row==null) {
                logger.error("Row data not found:#{}",rowIndex);
                return false;
            }
    ...
    public void setValueAt(Object value,int rowNr, int colNr) {
        ...
        if (row==null)
        {
            logger.warnPrintf("setValueAt(): Row index out of bound:%d\n",rowNr);
            return;
        }

        row.getAttribute(row.getAttributeName(colNr)).setObjectValue(value.toString());
   ...
   public Attribute[][] getAttributeData()
   {
       synchronized(rows)
       {
           ...
           for (int row=0;row<nrRows;row++)
           {
               ...
               String[] keys = this.getRow(row).rowAttributes.keySet().toArray(new String[0]);

               for (int col=0;col<keys.length;col++)
               {
                   attrs[row][col]=this.getAttribute(row,keys[col]).duplicate();
               }
           }
