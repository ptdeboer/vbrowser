 ...
 * - Checks Registry for default Java Installation. 
 * - JAVA_HOME specifies the java location. Default is to the one from the path.  
 */
...
const char* getRegistryJavaHome()
{
    ...
    // SystemRoot:
    value[0] = 0;
    DWORD bufferSize = (DWORD) MAX_TEXT_BUFFER;
    result = RegGetValue(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows NT\\CurrentVersion", "SystemRoot",
                         RRF_RT_ANY,
                         NULL,
                         (PVOID) &value,
                         &bufferSize);
    ...
    value[0] = 0;
    bufferSize = (DWORD) MAX_TEXT_BUFFER;
    result = RegGetValue(HKEY_LOCAL_MACHINE, javaCurrentRegKey.c_str(), "JavaHome",
                         RRF_RT_ANY,
                         NULL,
                         (PVOID) &value,
                         &bufferSize);
    ...
    const char *javaHome = Sduplicate(value);
    DEBUGPRINTF(" - JavaHome(2)='%s'\n",javaHome);
    return javaHome;
}
        ...
        int nCmdShow)
{
    ...
    DEBUGPRINTF(" - Registry JavaHome=%s\n",nonullstring(javaRegistryHome,"<undefined>"));

    // Environment variable JAVA_HOME overides Registry JavaHome (if set):
    if ((java_home==NULL) && (javaRegistryHome!=NULL))
    {
        java_home=Sduplicate(javaRegistryHome);
    }
    ...
    {
        char *message=Sappend("*** Jarlauncher Exception ***\n",
                              "javaw.exe not found:",
                              javaws_path);
